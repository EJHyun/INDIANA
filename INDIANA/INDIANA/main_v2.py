...
parser = argparse.ArgumentParser(description="Next-POI Recommendation (INDIANA)")
...
parser.add_argument("--num_neg", default=10, type=int,
                    help="K, the number of negative samples per positive (used only with --neg_sampling).")
parser.add_argument("--poi_temporal", action="store_true",
                    help="Score positives with KG-RNN-propagated POI embeddings (p_tilde_y), "
                         "rather than the static POI embedding (p_y). Default: off (static).")
parser.add_argument("--separate_poi_rnn", action="store_true",
                    help="Use a separate LSTM on the POI side (GRNN_p) instead of sharing the "
                         "user-side LSTM (GRNN_u). Default: off (shared LSTM).")
...
print(f"[options] neg_sampling={args.neg_sampling} (K={args.num_neg}), "
      f"poi_temporal={args.poi_temporal}, separate_poi_rnn={args.separate_poi_rnn}")
...
print('Loading graph...')
glist, _ = load_graphs('NYC_train.TKG', [0])
Train_Graph = glist[0]
...
model = model.GCRNN(
    User_cnt, POI_cnt, Cat_cnt * 2, emb_dim, User_cnt - 1, device,
    neg_sampling=args.neg_sampling,
    num_neg=args.num_neg,
    poi_temporal=args.poi_temporal,
    separate_poi_rnn=args.separate_poi_rnn,
)
    ...
    with torch.no_grad():
        ...
        for batch_it in tqdm(range(len(test_user_entid2) // test_batch_size if len(test_user_entid2) % test_batch_size == 0 else len(test_user_entid2) // test_batch_size + 1)):
            ...
            for user_id, UP_score in zip(test_user_entid2[prev_test_batch_cnt:test_batch_cnt], All_UP_score):
                ...
                POI_label_scores = UP_score[user_future_POI_index]  # True POI's score
                for cls_ in POI_label_scores:
                    gap = UP_score - cls_
                    filter_gap = gap[filter_poi[user_id]]
                    POI_ranks.append(len(gap[gap > 0]) - len(filter_gap[filter_gap > 0]) + 1)  # True POI's ranking
